...
//----------------------------------------------------------------------------
    ...
    void main() { gl_Position.xy = inA.xy; gl_Position.y *= -1.0; pos = inA.xy*0.5 + 0.5; gl_Position.zw = vec2(0.0,1.0); }
#endif
#ifdef PRG_1_F
    uniform sampler2D texA;
    noperspective varying vec2 pos;
    void main() { gl_FragColor = texture2D(texA, pos); }
#endif
...
Demo Scene 1K Shader is a modified version of "To the Road Of Ribbon" 
 from Inigo Quilez's Shader Toy,
  http://www.iquilezles.org/apps/shadertoy/
   Which is based on this 1K intro by FRequency, 
    http://www.pouet.net/prod.php?which=53939
...
float oa(vec3 q)
{
 return cos(q.x)+cos(q.y*1.15)+cos(q.z)+cos(q.x*0.75)*.2;
}

//Object B (ribbon)
float ob(vec3 q)
{
 return length(max(abs(q-vec3(cos(q.z*1.5)*.3,-.5+cos(q.z)*.3,.0))-vec3(.4,.01,time+103.),vec3(.0)));
}
...
void main()
{
 ...
 vec4 c=vec4(1.0);
 vec4 c2=vec4(1.0);
 vec3 org=vec3(sin(time)*.5,cos(time*.5)*.45+.45,time),dir=normalize(vec3(p.x*1.0,p.y*0.75,1.0)),q=org,pp;
 float d=.0;
 ...
 //Third raymarching (reflection 2)
 dir=reflect(dir,gn(q));
 q+=dir;
 for(int i=0;i<64;i++)
 {
 ...
 c2=max(dot(gn(q),vec3(.1,.1,.0)),.0)+vec4(.3,cos(time*.5)*.5+.5,sin(time*.5)*.5+.5,1.)*min(length(q-org)*.04,1.);
 //Ribbon Color
 if(oa(ppp)>ob(ppp))c2=mix(c2,vec4(cos(time*.3)*.5+.5,cos(time*.2)*.5+.5,sin(time*.3)*.5+.5,1.),.3);

 c = mix(c, c2, 0.125);

 //Final Color
 gl_FragColor = ((c+vec4(f))+(1.-min(pp.y+1.8,1.4))*vec4(.3,.2,.1,1.))*min(time*.5,0.75); }
